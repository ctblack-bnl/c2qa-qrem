...
**Version:** 0.13 — profile_loader bug fix; derived field fallbacks in profile generator; panel show/hide corrected (May 22)
**Date:** May 22, 2026
**Status:** Stages 1–4 complete and correctly operational. Stage 4 panel was silently broken until May 22 (profile_loader was dropping material sections — see Stage 4 notes). Corpus-average PUKs (Tier 3) are the next planned capability.
...
OpenQASM Circuit  +  Qubit Profile YAML  +  QEC Profile YAML
    ...
    [Stage 3]  ESTIMATE  (Physical World — Tier 1)
         ...
         |
    [Stage 4]  LOSS MECHANISM ATTRIBUTION  (complete May 21; correctly operational May 22)
               → T1 breakdown: TLS / quasiparticle / vortex motion / radiation
               → connects to materials properties (tan_delta → TLS, Tc → QP)
               → panel shown only when real TLS data (measured tan_delta, Qi+p_MS, or Q_TLS_0) present in profile
               → panel hidden when sliders touched
```
...
| **Phase 4** | Loss mechanism attribution: T1 → TLS/QP/vortex breakdown | ✅ Complete and correctly operational May 22. Stage C (UI) complete May 21. Critical profile_loader bug fixed May 22 — panel was silently using CLASS_DEFAULT for all profiles until then. |
...
The profile generator implements the `derived_X` fallback pattern for three material fields, consistent with `build_sqlite.py`:
- **T2**: `T2_echo_us` → `T2_ramsey_us` (tagged with which variant was used)
- **Qi**: `Qi_single_photon` → `Qi_internal`
- **tan_delta**: `tan_delta_effective_surface` → `loss_tangent_interface` → `loss_tangent_substrate` — new field in profile YAML as of May 22; drives T1 decomposition panel visibility
...
Tier 3 (corpus average) is the next planned capability — see continuity doc for design details. `compute_class_defaults.py` will query `records.db` grouped by `derived_material`, compute mean ± std across n≥3 samples, and write synthetic qubit profile YAMLs that appear in the QREM dropdown.
...
**Tier 2 derivation formulas (implemented in Stage 4 `t1_decomposition.py`):**
...
**Status: Stage 4 backend complete and operational. Panel correctly shows real material data as of May 22 (was masked by profile_loader bug until then).**
...
**Stage B (backend) — complete May 16.** `t1_decomposition.py` rewritten. Resonator excluded from Level 1 sum; junction uses single effective loss tangent; QP uses non-equilibrium floor. Validated against 4 Joshi 2026 qubits — all pass 5× threshold with correct frequency scaling (T1_pad_TLS ∝ 1/f). Key defaults: p_MS_pad=1.3e-4, tan_delta=1.6e-3 (β-Ta), T1_junction_TLS~1000µs, T1_QP=1000µs (non-equilibrium), T1_radiation=5000µs.
...
**Critical bug fixed May 22 — profile_loader.py.** `SECTION_KEYS["qubits"]` was missing `materials`, `device`, `surface_participation`. These sections were silently dropped on every profile load, meaning `t1_decomposition.py` never received any measured material data — all decompositions used CLASS_DEFAULT regardless of what was in the YAML. Fixed by adding the three missing sections. Stage 4 was not correctly operational until this fix.

**ε_ctrl bug fixed May 21.** ε_ctrl was previously derived from the selected corpus profile, producing ε_ctrl=0 for low-T2 profiles. Fixed: ε_ctrl always derived from `transmon_baseline_2026`.

**Panel visibility criterion:** Panel shown only when `tan_delta_provenance` is MEASURED or DERIVED. Hidden when all inputs are CLASS_DEFAULT. The `tan_delta` field in qubit profile YAMLs is now populated by `generate_qubit_profile.py` from the corpus record's best available surface loss measurement. In practice: profiles with measured `tan_delta_effective_surface` (e.g. Chang 2025, Joshi profiles) show the panel; profiles with no surface loss data (Wang, Bland, baseline) correctly hide it.

**Current TLS gating condition:** Panel shows if `tan_delta` non-null in profile, OR `Q_TLS_0` non-null, OR (`Qi` AND `p_MS_resonator`) both non-null. Currently only `tan_delta` gates the panel in practice — Q_TLS_0 and p_MS_resonator are rarely reported together. This will expand naturally as more papers with resonator geometry data are ingested.
...
*End of Specification Document v0.13*
*Updated May 22, 2026.*
