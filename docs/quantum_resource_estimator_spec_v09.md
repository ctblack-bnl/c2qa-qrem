...
**Version:** 0.9 — UI declutter, Stage 4 blocking note, Phase 5 clarification
**Date:** May 11, 2026
...
| **Phase 4** | Loss mechanism attribution: T1 → TLS/QP/vortex breakdown | Next — pending colleague conversation on loss model design |
...
|---|---|---|
| T1 (TLS contribution) | Qi, measurement frequency | `T1_TLS ≈ Qi / (2πf)` |
...
**Status: Design phase; first approved findings in `findings.jsonl` as of May 2026**
...
**Current status:** Design of the loss model is being worked out with a C2QA colleague. Implementation will follow that conversation. The Tier 2 formulas are well-established physics — the open question is the precise model architecture and which parameters to expose in the UI.

Once implemented, the Error Attribution panel gains a second level — not just ε_T1/ε_T2/ε_ctrl, but TLS/QP/vortex breakdown within ε_T1. Each contribution labeled with its individual provenance tier.
...
*End of Specification Document v0.9*
*Updated May 11, 2026.*
