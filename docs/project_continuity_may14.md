...
| 2 | **Publications Ingester** — reads PDFs, extracts structured materials data | Operational. Three-pass pipeline. 115 papers, 170 samples, ~1,390 catchall items, 100% profile coverage. max_tokens bumped to 32000; multi-device extraction prompt fixed. || 3 | **Materials Database + Explorer** — SQLite + browser UI | Live at https://c2qa-materials-explorer.onrender.com. Major UI update May 11 (see below). |
...
| 6 | **Materials-to-Device Mapping Layer** | `t1_decomposition.py` Stage 4 backend written (May 13). Two-level loss model with per-channel provenance. Companion YAMLs: `transmon_analytical_defaults.yaml`, `example_material_record.yaml`. Not yet integrated into estimator.py or UI. |
---
...
## Recent Completions (May 9–14, 2026)
...
### Ingester + Explorer — May 14, 2026

- **max_tokens bumped to 32000** — Pass 2 extraction was truncating on papers with many samples (e.g. Joshi 2026 with 11 qubits). Fix in `pipeline_ingest.py` line 124.
- **Multi-device extraction prompt fix** — added explicit instruction that each characterized qubit or resonator with distinct measured T1/T2/Qi values is a separate sample record, even if fabricated from the same film. Prevents collapsing of multi-qubit papers into single representative sample.
- **Joshi 2026 β-Ta paper ingested** — "Beta Tantalum Transmon Qubits with Quality Factors Approaching 10 Million" (arXiv:2603.13174). 12 samples: 11 individual qubits + 1 representative resonator record. PDF required ghostscript `/ebook` compression (48MB → 2.6MB) plus processed_ledger cleanup.
- **`derived_Qi` column** — best available Qi for plotting. Priority: `Qi_single_photon` → `Qi_internal_quality_factor`. Single-photon preferred (qubit operating regime, TLS unsaturated). Added to `build_sqlite.py` schema, compute block, and INSERT.
- **`derived_T2_us` column** — best available T2. Priority: `T2_echo_us` → `T2_ramsey_us`. Added same way.
- **`derived_` pattern established** — wherever the schema has multiple fields representing the same physical quantity measured different ways, `build_sqlite.py` gets a `derived_X` column with defined priority ordering. Explorer plots `derived_X`; detail drawer shows raw values. Future candidates: `derived_sheet_resistance`, `derived_resistivity`.
- **Explorer fields updated** — `derived_Qi` and `derived_T2_us` added to `ALL_NUMERIC_FIELDS` in `serve_materials.py`. Default y-axis changed to `derived_Qi` in `materials_explorer.html`.
- **records.db must be committed separately** — `git add .` from `ingester/` does not pick up `../data/ingested/records.db`. Must explicitly `git add ../data/ingested/records.db` before push for Render to update.

### Loss Channel Model — May 13–14, 2026

- **`t1_decomposition.py`** written and committed to `src/qrem/`. Implements full two-level loss model from `loss_channel_model_v2-5.md`. Per-channel provenance throughout. Model validation against measured T1 when available.
- **`transmon_analytical_defaults.yaml`** — class defaults for participation ratios (Wang 2015), interface loss tangents (Read 2023, sapphire), QP and vortex channel parameters.
- **`example_material_record.yaml`** — example input record for testing decomposition.
- **Not yet tested end-to-end** — CLI entry point exists (`python3 t1_decomposition.py <record.yaml> <defaults.yaml>`). Joshi 2026 Qubit-2 is the ideal first validation case (has measured T1, Tc, RRR, pMS, tan_delta).
- **Not yet integrated** into `estimator.py` or Baby QREM UI. Stage 4 backend exists; Part B drawer UI is next.
...
**Corpus state:** 115 papers processed, 170 samples, ~1,390 catchall items, 100% similarity profiles.

**Material breakdown (derived_material):** Ta (35+11=46 with β-Ta qubits), other (27), Al (16), unknown (13), NbSe2 (12), Re (12), Ta-Hf (12), PtSi (11), Mo3Al2C (5), NbN (5), TaN (2), Nb (1)

**Named column coverage:** Tc 39%, RRR 20%, Qi 17%, T1 13%

**New derived columns:** `derived_Qi` (34 samples), `derived_T2_us` (29 samples)
...
**1. Loss mechanism attribution (Stage 4 backend)** — `t1_decomposition.py` written (May 13). Two-level model: resonator as calibration (not loss channel), per-component TLS/QP/vortex decomposition, per-channel provenance. **Next step: run end-to-end test against Joshi 2026 Qubit-2, then integrate into `estimator.py`.**
