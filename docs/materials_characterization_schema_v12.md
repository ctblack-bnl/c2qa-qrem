...
| `qubit_frequency_GHz` | float | GHz | `well_known` | Operating point; collision avoidance. Named column in database (May 2026) — also required for pad TLS calculation: T1_pad_TLS = 1/(p_MS_pad × tan_delta × 2πf). |
...
**Changes in v0.12 (May 17, 2026):**
- Added `qubit_frequency_GHz` as named column in `build_sqlite.py` — required for pad TLS calculation in t1_decomposition.py
- Added `qubit_frequency_GHz` to extraction prompt (`prompts.py`) with explicit guidance
- Joshi 2026 and Bland 2025 re-ingested with updated prompt; qubit frequencies now populated as named fields
- Extended qubit profile YAML schema: `generate_qubit_profile.py` now writes `materials`, `device`, and `surface_participation` sections alongside `coherence` and `gates`
- Stage C complete: `t1_decomposition.py` integrated into `estimator.py`; T1 channel breakdown returned in every `/api/estimate` response
- Explorer material filter: Select All / None controls added
