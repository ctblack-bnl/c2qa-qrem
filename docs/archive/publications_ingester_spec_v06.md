...
## Specification Document v0.6

**Version:** 0.6 — Pass 3 similarity profiles, hybrid similarity search, material class Explorer
**Date:** April 27, 2026
**Status:** Operational — 97 papers, 155 samples, ~1,300 catchall items, 100% profile coverage
...
ingester/
  ...
  backfill_similarity_profiles.py — one-off Pass 3 backfill for existing records
...
## Current Corpus State (April 27, 2026)
...
Pass 3 sends Claude the completed PUK (all structured fields + full catchall) and asks it to generate an 8-dimension semantic profile using controlled vocabularies. One profile is generated per sample in the paper, keyed by `sample_id`. The full vocabulary is defined in Schema Block 6.
...
**`samples`** — one row per extracted sample: all schema fields, R vs T fields, derived quantities (prefixed `derived_`), similarity profile dimensions (prefixed `sim_`), `display_name` = `{first_author}_{year}_{sample_id}`.
...
**Explore** — Strip plot and scatter plot, any measurement vs material category or pair. Sidebar filters by `material_class` (from similarity profile) — stable ~10 categories regardless of corpus size. Click any point to open detail panel.

**Search** — Three sub-views: Table (all samples, key fields), Ranked (samples sorted by any measurement), Similar (similarity search results). Similarity result cards show matched profile tags (green ✓) and numeric field chips.

**Catchall** — All catchall items filtered by type (correlation, schema candidate, additional measurement, anomalous observation).

### Similarity Search

Triggered from any sample's detail panel via "Find similar →". Hybrid scoring:
- **75% profile score** — semantic match across 8 profile dimensions. Single fields: binary match. List fields: Jaccard similarity.
- **25% numeric score** — z-score normalized distance over overlapping numeric fields, weighted by √(overlap count).

Falls back gracefully: profile-only if no numeric overlap; numeric-only if no profile present. Result cards show `scoring_mode` when not hybrid.

### Slide-In Detail Panel

Full sample record: description, reference with DOI link, all measurements with confidence, catchall items. Includes **"⚙ Generate Qubit Profile"** and **"Find similar →"** actions.
...
- Measured fields (T1, T2, gate fidelity, readout fidelity, gate times) → labeled `[MEASURED]`
- Unmeasured fields → defaults from `transmon_baseline_2026.yaml`, labeled `[ASSUMED]`
- Full provenance block: DOI, authors, film material, substrate, date generated
...
|---|---|---|
| `additional_measurement` | Measurement with no schema field | ~850 |
| `schema_candidate` | Important parameter absent from schema | ~110 |
...
| Phase 2 — AI-assisted extraction | ✅ Complete | Three-pass pipeline (relevance, extraction, similarity profile), enriched prompt, Materials Explorer (Explore/Search/Catchall), derived quantities, deduplication, ingestion pipeline UI, Hardware Profile Updater, hybrid similarity search, public hosting on Render |
