...
| 3 | **Materials Database + Explorer** — SQLite + browser UI | ✅ Live at https://c2qa-materials-explorer.onrender.com. 191 samples (after 17 exclusions), 100% profile coverage. |
...
### T2_ramsey_us Column
`T2_ramsey_us` added as named column in `build_sqlite.py`, `serve_materials.py` (`ALL_NUMERIC_FIELDS`, `RAW_SOURCE_COLS`), and `materials_explorer.html` (`MEASUREMENT_FIELDS`). Extraction prompt (`prompts.py`) updated with explicit T2 disambiguation section (echo vs Ramsey notation). Re-ingested Joshi 2026, Wang 2026, Xia 2025. Appears in Explorer dropdown when populated.

### Per-Point Symbol Encoding for Derived Fields
Strip and scatter plots show solid vs open circles when a "best available" derived field is selected (`derived_Qi`, `derived_T2_us`, `derived_tan_delta`). `getDerivedPointStyles()` and `updateVariantLegend()` added to `materials_explorer.html`. Legend appears inside chart (absolute positioned, upper left, `top: 36px; left: 60px`) only when both variants are present in plotted data. Tooltip shows "Variant: …" line on hover. `serve_materials.py` returns `derived_Qi_source`, `derived_T2_us_source`, `derived_tan_delta_source` per sample.

### Sidecar Download Buttons
Sticky detail panel header now has `MD · JSON` format toggle and `↓` download button. MD = human-readable markdown summary (Reference, Sample Description, Measurements, Additional Data). JSON = full API response with confidence/source intact. `_currentDetailData` caches `{sample, catchall, displayName}` when panel opens. `openDetail()` updated to store cache.

### Strip Plot Jitter Fix
Jitter was value-relative (`pt.y * 0.02`), which corrupted plots with compressed non-zero ranges (e.g. fidelity 99–100% showing ±1 point scatter). Now range-relative: computes `yRange` across all plotted points, applies `yJitter = yRange * 0.01`. Affects all strip plot fields.

### derive.py Sheet Resistance Cascade
New `_derive_sheet_resistance_from_resistivity()`: Rs (Ω/□) = ρ (µΩ·cm) / t (nm) × 10. `_derive_kinetic_inductance()` accepts `Rs_override` parameter for cascade from upstream derivation. `derive_all()` now runs in dependency order: R vs T geometry → ρ+t sheet resistance (only if R vs T didn't fire) → kinetic inductance (passing derived Rs) → remaining. Kinetic inductance: 7→15 samples. Sheet resistance still ~8 — Bahrami/Yang resistivity still in catchall pending prompt fix.
...
### Explorer: Per-Point Symbol Encoding for Derived Fields

Strip and scatter plots show solid vs open circles when a "best available" derived field is selected (`derived_Qi`, `derived_T2_us`, `derived_tan_delta`). Legend appears inside chart (absolute positioned, upper left, `top: 36px; left: 60px`) only when both variants are present in the plotted data. Tooltip shows "Variant: …" line on hover. `serve_materials.py` returns `derived_Qi_source`, `derived_T2_us_source`, `derived_tan_delta_source` per sample.

### Explorer: Sidecar Download Buttons

Sticky detail panel header now has `MD · JSON` format toggle and `↓` download button. MD = human-readable markdown summary. JSON = full API response with confidence/source intact. `_currentDetailData` caches `{sample, catchall, displayName}` when panel opens.

### Explorer: Strip Plot Jitter Fix

Jitter was value-relative (`pt.y * 0.02`), corrupting plots with compressed non-zero ranges (e.g. fidelity 99–100% showing ±1 point scatter). Now range-relative: `yJitter = yRange * 0.01`. Affects all strip plot fields.

### Explorer: T2_ramsey_us Column

`T2_ramsey_us` added as named column in `build_sqlite.py`, `serve_materials.py`, and `materials_explorer.html`. Extraction prompt updated with explicit T2 disambiguation section (echo vs Ramsey notation). Re-ingested Joshi 2026, Wang 2026, Xia 2025. Appears in Explorer dropdown when populated.

### derive.py: Sheet Resistance Cascade

New `_derive_sheet_resistance_from_resistivity()`: Rs (Ω/□) = ρ (µΩ·cm) / t (nm) × 10. `_derive_kinetic_inductance()` accepts `Rs_override` for cascade. `derive_all()` now runs in dependency order. Kinetic inductance: 7→15 samples. Sheet resistance still ~8 — Bahrami/Yang resistivity still in catchall pending prompt fix.

### Explorer: Dropdown Reorder

`ALL_NUMERIC_FIELDS` in `serve_materials.py` reordered into four logical groups: Device performance / Superconducting properties / Microwave-loss / Fabrication. Removed `derived_BCS_gap_meV` and `derived_resistivity_uOhm_cm` from dropdown (kept in DB and detail drawer).
...
**6. Extraction prompt fix: normal_state_resistivity_uOhm_cm** — Still pending. Data is in catchall for Bahrami 2026 and Yang 2026. `build_sqlite.py` fallback and `derive.py` cascade are in place — once prompt is fixed and papers re-ingested, derived sheet resistance will jump from ~8 to ~22, and kinetic inductance from 15 to ~22.

**7. Prev/next navigation in sidecar footer** — Sticky footer with ← → arrows to cycle through samples in the current filtered set without closing the sidecar. `getFilteredSamples()` array provides the ordered list; step by index. Not yet implemented.
