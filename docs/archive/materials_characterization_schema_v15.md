...
### Version 0.15 — Fabrication process chemistry fields (Block 2.5); new fabrication_details catchall type; junction field renames; three planned derived columns for Explorer (July 2026)
**Date:** July 6, 2026
...
The catch-all is mandatory and is a first-class scientific output. It captures knowledge not yet formalized into schema fields and drives schema evolution. Author-stated correlations in the corpus are the primary evidence base for the QREM mapping layer, processed by the corpus mining pipeline.
...
- `fabrication_detail` — fabrication steps and process details not captured in named fields. The destination for resist brands and stack compositions, spin parameters, bake temperatures, anti-charging agents, packaging details, and other process information. Added v0.15.
...
**Current mining results:** See `findings.jsonl` for approved findings. The previously approved positive finding (Tc_K vs deposition_temperature in Ta-Hf, confidence 0.72) remains in `findings.jsonl` but is no longer supported by the live corpus — the correlation catchall item that seeded it is absent from the current ingestion of the PNAS version of Yang 2026. The data is present in the paper (Table 2) but was not extracted as a correlation item. Remaining findings inconclusive — expected at current corpus size.
...
**Domain rule — geometry independence:** Only intrinsic (geometry-independent) properties are promotable as measurement fields. Sheet kinetic inductance (Lk,sq) yes; total kinetic inductance (Lk) no. Critical current density (Jc-material, Jc-junction) yes; critical current (Ic) no.

**Domain rule relaxation for fabrication fields:** Fabrication process chemistry fields (Block 2.5) are promotable based on frequency and scientific relevance alone — they do not need to be geometry-independent, since process history is categorically different from intrinsic material properties.

**Fields promoted (May 9, 2026):** ✅ All three fields are named columns in the schema. Note: these were missing from `build_sqlite.py` despite being documented as promoted — restored May 22, 2026.
...
`generate_qubit_profile.py` implements the `derived_X` fallback pattern for three fields, consistent with `build_sqlite.py`:
- **T2**: `T2_echo_us` → `T2_ramsey_us` (tagged `[MEASURED — T2_ramsey]` when fallback used)
- **Qi**: `Qi_single_photon` → `Qi_internal` (tagged with variant used)
- **tan_delta**: `tan_delta_effective_surface` → `loss_tangent_interface` → `loss_tangent_substrate` (new field in profile YAML, drives T1 decomposition panel visibility)

The T1 loss channel breakdown panel in Baby QREM shows only when the profile contains a non-CLASS_DEFAULT tan_delta — i.e. when real surface loss data was measured for this sample.

### Derived Normalization Fields (Explorer Display)

`build_sqlite.py` computes normalization columns at build time for Explorer filtering and grouping. These are display/navigation aids — not schema fields in the PUK sense.
...
| `derived_resist_strip_family` | `resist_strip_chemistry` | AZ300T-family, NMP-family (1165/Remover PG), acetone-only, none, other | **Planned** — keyword matching at DB build time. Explorer strip chart group-by axis. |
| `derived_post_fab_treatment_family` | `post_fabrication_surface_treatment` | piranha+BOE, BOE-only, HF-only, acid-substitute, none, other | **Planned** — keyword matching at DB build time. Explorer strip chart group-by axis. |
| `derived_junction_vacuum_class` | `junction_chamber_vacuum` | UHV, HV, unknown | **Planned** — keyword matching at DB build time. Explorer strip chart group-by axis. |

**Note on planned derived columns:** The three fabrication-derived columns (`derived_resist_strip_family`, `derived_post_fab_treatment_family`, `derived_junction_vacuum_class`) are designed but not yet implemented in `build_sqlite.py`. They will be added after the full corpus re-ingestion with the v0.15 prompt confirms sufficient population.
...
[2] SAMPLE DESCRIPTION      — what material, how made, R vs T geometry
    ...
    2.5 Fabrication Process Chemistry (NEW v0.15)
...
| `junction_oxidation_protocol` | string | If yes | Full oxidation sequence: gas, pressure, duration. Multi-step sequences common — capture all steps. Renamed from `junction_oxidation_conditions` in v0.15. |
| `junction_resistance_normal_Ohm` | float | If yes | Normal-state resistance in Ohms |
| `junction_pre_deposition_surface_treatment` | string | No | Surface preparation immediately before junction metal deposition. Capture both ex-situ treatments (BOE, descum — outside chamber) and in-situ treatments (Ar ion milling — inside chamber, with bias voltage, duration, angle). Explicit absence ("no ion milling") is a valid and important value. Renamed and expanded from `junction_vacuum_condition` in v0.15. |
| `junction_developer` | string | No | Resist developer for EBL junction patterning. Two approaches in literature: room-temperature MIBK:IPA (conventional) and cold IPA:DI water (sub-zero to ~6°C). Include composition, temperature, duration. |
| `junction_chamber_vacuum` | string | No | Vacuum quality of junction metal evaporation chamber. HV vs UHV is a primary determinant of T2E. Include vacuum level, base pressure, system name if stated. |
| `junction_liftoff_chemistry` | string | No | Resist removal after junction evaporation. Distinct from resist_strip_chemistry — liftoff dissolves resist under unwanted metal. Include solvent, temperature, duration. |
...
Fabrication process chemistry fields capture the detailed process sequence from substrate preparation through final surface treatment. These fields are free-text strings — the vocabulary is too diverse and lab-specific for controlled enums. All fields are optional and sparse: only populate what the paper actually reports.

**Scientific motivation:** Resist strip chemistry, post-fabrication surface treatment, and junction deposition vacuum have all been shown to directly influence qubit T1 and T2. These fields enable corpus-level analysis of process-to-performance connections across labs.

**Promotion rule:** These fields follow a relaxed promotion criterion — frequency and scientific relevance, not geometry independence. Process history is categorically different from intrinsic material properties.

**Base-layer processing fields** (substrate → deposition → etch → strip → post-fab → dicing):

| Field | Type | Description |
|---|---|---|
| `substrate_prep_before_deposition` | string | Complete sequence of steps applied to the substrate immediately before loading into the film deposition chamber. Include: oxide removal chemistry (HF, BOE, piranha), cleaning sequence, transfer time constraint, getter steps. Note: in-situ heating goes in `in_situ_substrate_bake_temperature_C` AND may be noted here for context. |
| `in_situ_substrate_bake_temperature_C` | float | Temperature of in situ substrate heating inside the deposition chamber immediately before film deposition, in °C. Distinct from `deposition_temperature_C` (temperature during film growth) — this is a pre-deposition bake for surface desorption. |
| `film_deposition_conditions` | string | Detailed deposition conditions not captured by `deposition_method` and `deposition_pressure_torr`. Include: deposition rate (nm/s or nm/min), sputtering power (W), gas pressure, gas flow ratios, system name, target purity. |
| `film_etch_chemistry` | string | Etch process used to pattern the superconducting film. Include: etch type (dry/wet), gas chemistry, ICP power, RF bias voltage, chamber pressure, gas flow ratios, chamber conditioning runs. |
| `resist_strip_chemistry` | string | Solvent/chemical process to remove photoresist after subtractive (dry etch) patterning. Distinct from `junction_liftoff_chemistry`. Include: solvent identity, temperature, duration, sonication steps, O2 plasma descum. Note: strip chemistry choice directly determines Qi in halogen-etched Nb/Si systems — always extract if reported. |
| `post_fabrication_surface_treatment` | string | Final surface treatment after patterning and resist strip, before measurement or junction fabrication. "none" is a valid and important value. Include: chemical identity, concentration, temperature, duration, full sequence. Note: material-specific — some films tolerate piranha/BOE, others require substitutes. |
| `dicing_protocol` | string | Dicing process and associated wet chemistry. Dicing is the last wet environment before measurement. Include: resist used for protection, dicing parameters, strip chemistry after dicing. |

**Junction-specific processing fields** (add to Block 2.3 junction fields above):

See `junction_pre_deposition_surface_treatment`, `junction_developer`, `junction_chamber_vacuum`, `junction_oxidation_protocol`, and `junction_liftoff_chemistry` in Block 2.3.

**What goes in the `fabrication_details` catchall instead:** Resist brands and stack compositions (including unusual interlayer materials such as Ge hard mask layers), spin parameters, bake temperatures, anti-charging agents, HMDS adhesion promoter steps, ion mill bias details, sonication sequences not part of named steps, measurement packaging (commercial package name and model, e.g. QCage, SCALINQ SC-3D, or description of custom packaging).
...
| `p_MS_resonator` | float | dimensionless | `well_known` | Surface participation ratio of the metal-substrate interface for the resonator. Required to invert Q_TLS,0 → tan_delta: tan_delta = 1/(Q_TLS,0 × p_MS_resonator). From FEM simulation or geometry lookup. |
| `p_MS_pad` | float | dimensionless | `well_known` | Surface participation ratio of the qubit capacitor pads. Required to convert tan_delta → T1_pad_TLS: T1_pad_TLS = 1/(p_MS_pad × tan_delta × 2πf). Much smaller than p_MS_resonator by design. From HFSS simulation. |
...
**Fabrication details:** Fabrication steps and process information not captured in Block 2.5 named fields. The long tail of process detail: resist brands and stack compositions (including unusual interlayer materials such as Ge hard mask layers), spin parameters, bake temperatures, anti-charging agents, HMDS adhesion promoter steps, ion mill bias details beyond what fits in named fields, measurement packaging (commercial package name and model, e.g. QCage, SCALINQ SC-3D, or description of custom packaging). No `suspected_relevance` required — relevance assessed during corpus mining.
...
Microwave and device performance:
  ...
  is required. TLS saturation at qubit operating power means measured T1 may exceed
...
Fabrication process:
  Resist strip chemistry → halogen residue removal → Qi (in BCl3/Cl2-etched Nb/Si systems,
    AZ300T removes residues that NMP-family baths do not)
  Post-fabrication surface treatment → native oxide control → TLS loss → T1
  Junction deposition vacuum (HV vs UHV) → junction barrier quality → T2E
  Substrate preparation (RCA, BOE, HF, thermal oxidation) → MS interface quality → TLS loss
...
fabrication_details:
  - description: "<fabrication step or process detail not in named fields>"
...
- Fabrication detail items → process-to-performance hypothesis seeding (future mining extension)
...
**Changes in v0.15 (July 2026):**
- Added Block 2.5 — Fabrication Process Chemistry: seven named free-text fields for base-layer processing (`substrate_prep_before_deposition`, `in_situ_substrate_bake_temperature_C`, `film_deposition_conditions`, `film_etch_chemistry`, `resist_strip_chemistry`, `post_fabrication_surface_treatment`, `dicing_protocol`).
- Added five junction process chemistry fields to Block 2.3: `junction_pre_deposition_surface_treatment` (renamed and expanded from `junction_vacuum_condition`), `junction_developer`, `junction_chamber_vacuum`, `junction_liftoff_chemistry`. `junction_oxidation_conditions` renamed to `junction_oxidation_protocol`.
- Added `fabrication_detail` as a fourth catchall type alongside `additional_measurement`, `anomalous_observation`, and `correlation`. `free_notes` is no longer the destination for fabrication details.
- Added three planned derived columns to the normalization table: `derived_resist_strip_family`, `derived_post_fab_treatment_family`, `derived_junction_vacuum_class` — to be implemented in `build_sqlite.py` after corpus re-ingestion confirms sufficient population.
- Updated Known Materials-to-Device Connections in Block 5 to include fabrication process connections (resist strip chemistry → Qi, junction vacuum → T2E, substrate prep → TLS loss).
- Relaxed promotion domain rule for fabrication fields: process history fields are promotable on frequency + scientific relevance, not the geometry-independence criterion that applies to material property fields.
- Prompt updated to v4 (`prompts.py`) with FABRICATION PROCESS CHEMISTRY section. Tested on Joshi 2026 (14 samples, Ta transmon) and Olszewski 2026 (14 samples, Nb/Si process comparison) — extraction quality high, all named fields correctly populated where reported.
- Full re-ingestion of all 35 papers that passed triage is pending with the v4 prompt.

**Changes in v0.14 (May 2026):**
- `generate_qubit_profile.py` now implements `derived_X` fallback pattern for T2 (echo→Ramsey), Qi (single-photon→internal), and tan_delta (three-way fallback). Each field tagged with which variant was used. `tan_delta` is a new field in the profile YAML — was completely absent before.
- `mean_free_path_nm`, `vortex_activation_temperature_K`, `kinetic_inductance_sheet_pH_sq` restored as named columns in `build_sqlite.py`. These were documented as promoted in v0.7 but were missing from the DB implementation until May 22.
- `pipeline_mining.py` FIELD_MAP updated: `"qtls"` now correctly maps to `Q_TLS_0` (was wrongly `Qi_internal`); `"loss tangent"` and variants map to `derived_tan_delta`; `p_MS_resonator`, `resonator_gap_width_um`, `T2_ramsey_us`, `qubit_frequency_GHz` updated from `json:` paths to named columns.
- Mining run May 22: no new positive findings. Previously approved Ta-Hf finding no longer supported by live corpus (correlation item missing from current Yang 2026 ingestion).
