...
**Version:** 0.11 — Stage 4 backend validated against Joshi 2026 (May 16)
**Date:** May 16, 2026
**Status:** Stages 1–3 complete; Stage 4 backend validated, not yet integrated into estimator; UI operational; materials pipeline connected
...
| **Phase 4** | Loss mechanism attribution: T1 → TLS/QP/vortex breakdown | ✅ Backend validated May 16. `t1_decomposition.py` rewritten (Stage B). Validated against 4 Joshi 2026 qubits — all pass 5x threshold, correct frequency scaling. Not yet integrated into `estimator.py` or UI. Next: Stage C integration. |
...
**Current status (May 16):** `t1_decomposition.py` rewritten (Stage B complete). All four changes implemented: resonator excluded from Level 1 sum; junction uses single effective loss tangent; units fix (GHz → cycles/µs); QP uses non-equilibrium floor. Validated against 4 Joshi 2026 qubits — all pass 5x threshold with correct frequency scaling (T1_pad_TLS ∝ 1/f). Key defaults anchored to Joshi geometry: p_MS_pad=1.3e-4, tan_delta=1.6e-3 (β-Ta), T1_junction_TLS~1000µs, T1_QP=1000µs (non-equilibrium), T1_radiation=5000µs. Model correctly predicts below measured T1 for best qubits — TLS saturation at qubit operating power is expected and documented. **Next: Stage C — integrate into `estimator.py`.**
...
*End of Specification Document v0.11*
*Updated May 16, 2026.*
