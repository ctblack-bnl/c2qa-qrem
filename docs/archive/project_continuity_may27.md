...
| # | Component | Status |
|---|---|---|
| 1 | **Baby QREM** — circuit + hardware profile → resource estimate | ✅ Stages 1–4 complete. T1 loss channel breakdown panel operational. Panel shows only when real TLS data (measured tan_delta, Qi, or Q_TLS_0) is present in profile — hides for CLASS_DEFAULT. ε_ctrl fixed to controls baseline. Live at https://c2qa-baby-qrem.onrender.com |
...
## Recent Completions (May 27, 2026)

### Repo Restructure — Complete

The repository has been reorganized into two audience-specific subtrees to make it easier to share with collaborators:

**New directory structure:**
```
explorer/          ← previously ingester/ (renamed; all ingester code here)
qrem/              ← previously src/qrem/ + scripts/ + tests/
  scripts/         ← previously scripts/ at repo root
  tests/           ← previously tests/ at repo root
  hardware_profiles/
    qubits/
    material_defaults/   ← previously mapping_models/
    error_correction/
archive/           ← previously materials/ (dead code, kept for reference)
data/              ← unchanged
docs/              ← unchanged
```

**Three audience-specific READMEs added:**
- Root `README.md` — one-page orientation, points to live Explorer URL, links to each subtree
- `explorer/README.md` — for materials scientists / Explorer contributors
- `qrem/README.md` — for QEC/algorithms contributors, with plugin points for Stim, HetArch, Arquin, Qualtran

**Path fixes applied during restructure:**
- `qrem/scripts/serve.py` — REPO_ROOT now `parent.parent.parent` (was `parent.parent`); STATIC_DIR updated to `explorer/static`; serving directory set to `qrem/`; root URL `/` redirects to `/scripts/qrem_ui.html`
- `qrem/estimator.py` — defaults path updated to `hardware_profiles/material_defaults/transmon_general_defaults.yaml`
- `qrem/profile_loader.py` — hardware_profiles path updated
- `explorer/serve_materials.py` — PROFILES_DIR updated to `qrem/hardware_profiles`
- `explorer/generate_qubit_profile.py` — default profiles-dir updated to `../qrem/hardware_profiles`
- `explorer/compute_class_defaults.py` — default profiles-dir updated
- All qubit YAML `defaults_path` fields updated from `../mapping_models/transmon_analytical_defaults.yaml` to `../material_defaults/transmon_general_defaults.yaml`

**Both services deployed on Render:**
- Materials Explorer: https://c2qa-materials-explorer.onrender.com (existing, start command updated to `python3 explorer/serve_materials.py`)
- Baby QREM: https://c2qa-baby-qrem.onrender.com (new, start command `python3 qrem/scripts/serve.py`)

### Corpus-Average PUKs — Complete (May 23)
...
- `qrem/hardware_profiles/qubits/{Material}_corpus_average.yaml` — device performance profile with corpus-averaged T1, T2, gate fidelities (when N≥3). Appears in Baby QREM qubit dropdown.
- `qrem/hardware_profiles/material_defaults/{Material}_material_defaults.yaml` — material property defaults for `t1_decomposition.py`.
...
1. **`tan_delta_effective_surface` from Γ_surf:** Papers using participation matrix inversion report Γ_surf directly in loss factor tables. This IS tan_delta_effective_surface.
2. **`p_MS_pad` from participation tables:** Participation tables with a "Transmon" column contain p_MS_pad in the transmon column.
...
1. **Regenerate all corpus profiles** — click "Generate Qubit Profile" in Explorer for each sample, or write a batch script that calls `generate_qubit_profile.py` for all samples in the DB.
...
- `FIELD_MAP` in `explorer/pipeline_mining.py`
- `DERIVED_MATERIAL_FIELDS` / `MATERIAL_FIELD_MAPPINGS` in `explorer/generate_qubit_profile.py`
- Column definitions in `explorer/build_sqlite.py`
- Priority chains in `qrem/t1_decomposition.py`
...
- Resist whack-a-mole prompt changes. Each addition should be general enough to fire on multiple papers. The Γ_surf fix covers all participation matrix papers (Wang, Ganjam, and future Yale-style papers).
