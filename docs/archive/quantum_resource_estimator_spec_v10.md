...
**Version:** 0.10 — UI declutter, Stage 4 backend written (May 14)
**Date:** May 14, 2026
**Status:** Stages 1–3 complete; Stage 4 backend written, not yet integrated; UI operational; materials pipeline connected
...
| **Phase 4** | Loss mechanism attribution: T1 → TLS/QP/vortex breakdown | Backend written May 13 — `t1_decomposition.py` in `src/qrem/`. Two-level model per `loss_channel_model_v2-5.md`. Not yet integrated into `estimator.py` or UI. Next: end-to-end test then wire into estimator. |
| **Phase 4b** | Part B material deep dive drawer | Planned — prerequisite: Phase 4 backend |
| **Phase 5** | RRR/Qi sensitivity: upstream material property sliders → T1 | Planned — depends on Phase 4, not on corpus mining findings |
...
src/qrem/
  ...
  hardware_profiles/
    ...
    mapping_models/               — T1 decomposition model inputs (new May 13)
      transmon_analytical_defaults.yaml  — class defaults: participation ratios (Wang 2015),
                                           interface loss tangents (Read 2023, sapphire),
                                           QP and vortex parameters
      example_material_record.yaml       — example input for testing decomposition
...
Gate Error Decomposition bar — ε_T1 / ε_T2 / ε_ctrl with fractions and stacked bar. ε_T1 row styled with `▸` affordance as trigger for Part B material deep dive drawer (not yet built). Provenance line showing [MEASURED] vs [ASSUMED] fields.
...
Every input to Baby QREM follows a four-tier fallback:
...
| 3 | Typical value for this material class | `[CLASS DEFAULT]` | Ta films typically achieve T1 ~100µs |
| 4 | Baseline profile assumption | `[ASSUMED]` | `transmon_baseline_2026.yaml` default |
...
|---|---|---|
| T1 (TLS contribution) | Qi, measurement frequency, participation ratios | Resonator Qi → tan(δ) per interface → pad T1_TLS via participation ratios. See `loss_channel_model_v2-5.md` for full two-level structure. Simple proxy: `T1_TLS ≈ Qi / (2πf)` (aggregate, not component-resolved). |
...
**Status: Stage 4 backend written (`t1_decomposition.py`); not yet integrated into estimator. First approved findings in `findings.jsonl` as of May 2026.**
...
**This does not require corpus mining findings to implement.** Standard analytical formulas exist for each mechanism (see Tier 2 fallback table above). The implementation uses whatever inputs are available from the corpus record, falling back through the tier hierarchy for missing values.

**Current status (May 14):** `t1_decomposition.py` written and committed. Implements full two-level model from `loss_channel_model_v2-5.md` incorporating feedback from Mingzhao Liu (C2QA colleague): resonator is a calibration measurement not a qubit loss channel; participation ratios require FEM simulation; junction TLS is independent of resonator Qi. Companion YAML files provide class defaults. **Not yet integrated into `estimator.py` or UI.** Next step: run end-to-end test against Joshi 2026 Qubit-2 (ideal validation case: measured T1=585µs, Tc=0.7K, RRR=1.02, pMS=1.3×10⁻⁴, tan_δ=1.6×10⁻³), then wire into estimator.Once implemented, the Error Attribution panel gains a second level — not just ε_T1/ε_T2/ε_ctrl, but TLS/QP/vortex breakdown within ε_T1. Each contribution labeled with its individual provenance tier.

**Stage 4b — Part B drawer:** Bottom drawer triggered by clicking ε_T1 in the Error Attribution panel (affordance already in place). Shows T1 decomposed into channels with per-channel provenance. Upstream material property sliders (Qi, Tc, mean free path) update main page metrics and staircase in real time. Staircase must stay visible — it shows code distance threshold crossings. Bottom drawer pushes content up, does not overlay. Prerequisite: Stage 4 backend must exist first.

**Stage 5 (planned):** Add upstream material property sliders (RRR, Qi, loss tangent) that feed through the Tier 2 mapping functions into T1/T2. The T1 sensitivity curve becomes a material property sensitivity curve — "if I improve RRR from 10 to 50, what happens to my physical qubit count?" Depends on Stage 4 backend. **Does not require corpus mining findings** — Tier 2 physics formulas are implementable now; mining findings will validate and refine them later.
...
*End of Specification Document v0.10*
*Updated May 14, 2026.*
