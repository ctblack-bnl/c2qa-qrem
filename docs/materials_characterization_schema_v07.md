...
### Version 0.7 — Corpus Mining Pipeline, Schema Evolution Redesign, Promoted Fields
...
### Current Database State (April 28, 2026)
...
**Current mining results (April 28):** 41 correlations found, 13 hypotheses mapped, 3 findings produced. All findings are negative or inconclusive — expected at current corpus size for key field pairs. Key insight from Phase B: cross-material hypothesis testing is confounded by material identity; per-material-class analysis is the correct unit of analysis.
...
2. Human approval via Stage 4 schema evolution UI
...
**Fields confirmed ready for promotion (April 28):**
...
Future schema additions: `derived_material_Jc_A_m2` and `derived_junction_Jc_uA_um2` (new `derive.py` functions). New Block 2 fields needed: `film_width_um` for material Jc derivation.
...
| `schema_version` | string | Yes | e.g. `0.7` |
...
| `film_material_other` | string | If other | Chemical formula e.g. `Ta-Hf (83:17)`, `TaN`, `NbSe2`, `PtSi` |
...
| `kinetic_inductance_sheet_pH_sq` | float | pH/sq | `well_known` | Resonator frequency; superinductor performance. Sheet (per-square) value only — geometry-independent. |
| `mean_free_path_nm` | float | nm | `well_known` | Clean vs dirty superconducting limit; vortex behavior. l > ξ → clean limit, vortex motion dominant. |
| `vortex_activation_temperature_K` | float | K | `approximate` | Vortex motion loss channel; microwave loss at operating temperature |
...
**Version:** `profile_version: "1.0"` — bump when vocabulary changes. Stale profiles regenerated via `backfill_similarity_profiles.py`.
...
|---|---|---|
| `material_class` | single | `niobium`, `aluminum`, `tantalum`, `rhenium`, `titanium_nitride`, `niobium_nitride`, `niobium_titanium_nitride`, `tantalum_nitride`, `tantalum_hafnium`, `niobium_diselenide`, `platinum_silicide`, `other` |
...
**Explorer integration:** The `material_class` field populates the sidebar filter in the Explorer. Added `niobium_diselenide` and `platinum_silicide` to vocabulary to reflect new materials appearing in the corpus.
