...
**Version:** 0.8 — Tiered fallback hierarchy, loss mechanism attribution design
**Date:** May 5, 2026
...
Once implemented, the Error Attribution panel gains a second level — not just ε_T1/ε_T2/ε_ctrl, but TLS/QP/vortex breakdown within ε_T1. Each contribution is labeled with its provenance tier so the researcher knows which are measured vs assumed.

**Stage 5 (planned):** Add upstream material property sliders (RRR, Qi, loss tangent) that feed through the Tier 2 mapping functions into T1/T2. The T1 sensitivity curve becomes a material property sensitivity curve — "if I improve RRR from 10 to 50, what happens to my physical qubit count?"
